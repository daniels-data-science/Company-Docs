...
Overall, if we consider the ready-to-deploy model as the contribution, then Guardian members will need to take an extremely long detour to get there. This is the reason why Guardian AI’s work has been independent from the rest of Guardian’s. The detour is so long that the two no longer have anything in common apart from the end goal. At the time of writing, converging on this end goal within the foreseeable future seems far-fetched, considering the amount of work we still ahead before we integrate into the app.
