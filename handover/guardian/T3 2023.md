...
The ML-oriented instructions for key feature ONE will be summarised in the subsections below. However, to keep the handover document clean, please refer to [Guardian AI Document]('https://docs.google.com/document/d/1O0OyA2T06X0iOK-fld8PQn7jwCY1-Rdr/edit?usp=sharing&ouid=110616284047482890831&rtpof=true&sd=true")


### AI Pipeline's Logistics
Our AI-driven system presents a logistical challenge from the large amount of preliminary work. In short, the logic and process is listed below. We shall work *backwards* from its successful implemention to its foundation, step-by-step:
...
* This pipeline is constantly receiving real-time streams of signalling states while also constantly giving real-time updates on the current activity. This means that while we are engineering the system, we need to make sure the latency (processing times) remains low.
...
* The aformentioned stages and components require at least a high-level understanding of the data in the pipeline. ie. Knowing what a signalling state means.
* There should also be some basic intuition behind the factors that influences signalling states (very high-level only).
Therefore, it is imperative that new Guardian AI engineers undergo pre-requisite learnings streamlined into a stage called **Data Familiarisation** before working on any pipeline components.
**DO NOT SKIP DATA FAMILIARISATION JUST BECAUSE IT DOES NOT GIVE TANGIBLE RESULTS. YOU MUST KNOW WHAT THE DATA MEANS BEFORE TRYING TO BUILD A MODEL. THIS DIRECTIVE IS *EMPHASISED*  BY DR. B. NAKISA. PREVIOUS ATTEMPTS TO SKIP THIS STAGE HAS WASTED ENTIRE MONTHS. DO NOT REMOVE THIS WARNING.**
